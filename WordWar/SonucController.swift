...
class SonucController: UIViewController {
    ...
    func sonucGetir(homeSonuc:Bool) -> Void {
        
        skorGetir(uid: user.id!,scoreText: homeScore)
        skorGetir(uid: rakip.id!,scoreText: awayScore)
        if homeSonuc {
            homeResult.text="Kazanan"
            awayResult.text="Kaybeden"
        }else{
            homeResult.text="Kaybeden"
            awayResult.text="Kazanan"
        }
    }
